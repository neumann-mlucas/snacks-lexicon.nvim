...
local function dns_ok(host, timeout_ms)
  ...
  vim.wait(timeout_ms or 2000, function() return result ~= nil end, 50)
  ...
  for key, cfg in pairs(lex.config.languages) do
    ...
    if wf then
      ...
      vim.health.warn(
        ...
        { "Tried: " .. paths,
          "Install a system dict package or override `languages." .. key .. ".word_files`" }
      )
    end
  end
